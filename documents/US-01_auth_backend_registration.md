...
2.Critères d'acceptations (AC)
    ...
    -1. reception de la requete: l'API doit être capable de transmetre la requete au serveur ( serveur active ou en marche) code 200
    -2. Validation des données: l'API doit retourner une erreur 409 si l'email est déjà utilisée.
    -3. Sécurité (hashage): le mot de passe ne doit pas être enregistré en clair. (bcrypt)
    -4. Sécurité (serveur): l'API doit retourner une erreur 500 si il y'a un problème de connexion à MongoDB
    -5. Enregistrement:  un nouveau utilsateur a été créer,  API doit retourner : un message de success "utilisateur enregistré avec success",  un code 201 et les informations du l'utilisateur sans le mot de passe.
...
3.BDD (gherkin):
        ...
        Scénario 1: Disponibilité du serveur (AC 200)
        Given l'endpoint de creation de compte est disponible
        Then le systèm envoie le code 200 et le message de success "le servuer est active".

        Scénario 2: Echec d'inscription l'email est déjà utilisé (AC 409)
        Given l'endpoint de creation de compte est disponible
        ...
        Scénario 3: Le mot de passe ne doit être transforme pour assurer la sécurité.
        Given l'endpoint de creation de compte est disponible
        ...
        Scénario 4: Echec d'inscription, le serveur est hors usage. (AC 500)
        Given l'endpoint de creation de compte est disponible
        ...
        Scénario 5: Inscription Reussie, quand l'utilsateur envoie les information correct (AC 201)
        Given l'endpoint de creation de compte est disponible
